...
import { StockRequest } from '../../core/models';
import { createPager, PAGE_SIZE_OPTIONS } from '../../shared/pager';

@Component({
  selector: 'app-request-items-dialog',
  ...
  template: `
    ...
    <mat-dialog-content>
      <p class="sub">{{ data.requestNumber }} — {{ data.createdAt | date:'dd-MM-yyyy' }}</p>
      ...
      <table mat-table [dataSource]="pager.paged()" class="full">
        ...
        <ng-container matColumnDef="price">
          ...
          <td mat-cell *matCellDef="let it">{{ it.unitPrice | currency:'INR' }}</td>
        </ng-container>
        <ng-container matColumnDef="line">
          <th mat-header-cell *matHeaderCellDef>Line Total</th>
          <td mat-cell *matCellDef="let it">{{ it.lineTotal | currency:'INR' }}</td>
        </ng-container>
        <tr mat-header-row *matHeaderRowDef="columns"></tr>
        <tr mat-row *matRowDef="let row; columns: columns"></tr>
      </table>
      @if (!data.items.length) { <div class="empty">No items in this request.</div> }
...
export class RequestItemsDialog {
  data = inject<StockRequest>(MAT_DIALOG_DATA);
  columns = ['item', 'qty', 'price', 'line'];
