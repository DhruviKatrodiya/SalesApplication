...
public class ReportsController : OwnedControllerBase
{
        ...
        [FromQuery] int page = 1, [FromQuery] int pageSize = 5)
    {
        ...
        var query = _db.Orders.Where(o => o.SalesmanId == uid && o.IsActive && o.Status != OrderStatus.Cancelled && o.OrderDate.Year == year);
        ...
        [FromQuery] int page = 1, [FromQuery] int pageSize = 5)
    {
        ...
        var orders = await _db.Orders
            .Where(o => o.SalesmanId == uid && o.IsActive && o.Status != OrderStatus.Cancelled && o.OrderDate.Year == year && o.OrderDate.Month == month)
            .ToListAsync();
    ...
    {
        var uid = CurrentUserId;
        var orders = await _db.Orders.Where(o => o.SalesmanId == uid && o.IsActive && o.Status != OrderStatus.Cancelled && o.OrderDate.Year == year).ToListAsync();
        var rows = orders
            .GroupBy(o => o.OrderDate.Month)
            .OrderBy(g => g.Key)
            .Select(g => new ReportRow(
