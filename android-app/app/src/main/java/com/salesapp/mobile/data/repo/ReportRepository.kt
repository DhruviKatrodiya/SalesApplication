...
/**
 * Order-based reporting, mirroring ReportsController. Only active, non-cancelled orders count.
...
class ReportRepository {
    ...
    suspend fun monthly(year: Int, month: Int?): ReportSummary = Db.withConnection { conn ->
        val extra = if (month != null) " AND MONTH(o.OrderDate) = ?" else ""
        val filter = "${baseWhere()} AND YEAR(o.OrderDate) = ?$extra"
        val args = buildList { add(Session.userId); add(year); if (month != null) add(month) }
        val rows = groupRows(conn, filter, args, "MONTH(o.OrderDate)") { monthNames[it - 1] }
    ...
    suspend fun daily(year: Int, month: Int): ReportSummary = Db.withConnection { conn ->
        val filter = "${baseWhere()} AND YEAR(o.OrderDate) = ? AND MONTH(o.OrderDate) = ?"
        val args = listOf(Session.userId, year, month)
        val rows = groupRows(conn, filter, args, "DAY(o.OrderDate)") { "%02d %s".format(it, monthNames[month - 1]) }
                ...
                buildList {
                    while (rs.next()) add(
                        CustomerReportRow(
                            ...
                            totalOrders = rs.getInt("TotalOrders"), pendingOrders = rs.getInt("Pending"),
                            deliveredOrders = rs.getInt("Delivered"),
                            totalAmount = rs.getBigDecimal("TotalAmount")?.toDouble() ?: 0.0,
                            paidAmount = rs.getBigDecimal("Paid")?.toDouble() ?: 0.0,
                            remainingAmount = rs.getBigDecimal("Remaining")?.toDouble() ?: 0.0,
                        )
                    )
                }
            }
        ...
        return conn.prepareStatement(sql).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                buildList {
                    while (rs.next()) add(
                        ReportRow(
                            label = label(rs.getInt("Grp")),
                            orderCount = rs.getInt("Cnt"),
                            totalAmount = rs.getBigDecimal("Total")?.toDouble() ?: 0.0,
                            paidAmount = rs.getBigDecimal("Paid")?.toDouble() ?: 0.0,
                            remainingAmount = rs.getBigDecimal("Remaining")?.toDouble() ?: 0.0,
                        )
                    )
        ...
        return conn.prepareStatement(sql).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                ...
                ReportSummary(
                    ...
                    totalAmount = rs.getBigDecimal("Total")?.toDouble() ?: 0.0,
                    totalPaid = rs.getBigDecimal("Paid")?.toDouble() ?: 0.0,
                    totalRemaining = rs.getBigDecimal("Remaining")?.toDouble() ?: 0.0,
