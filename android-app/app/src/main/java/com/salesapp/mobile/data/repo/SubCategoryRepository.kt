...
/** CRUD for [SubCategory] (local SQLite): user-scoped, category join + validation, item counts, soft-delete. */
class SubCategoryRepository {
    ...
    suspend fun list(
        ...
        if (active != "all") { where.append(" AND s.IsActive = ?"); args.add(if (active == "inactive") 0 else 1) }
        if (categoryId != null) { where.append(" AND s.CategoryId = ?"); args.add(categoryId) }
        if (!search.isNullOrBlank()) { where.append(" AND s.Name LIKE ?"); args.add("%${search.trim()}%") }
        ...
        val items = conn.prepareStatement(sql).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                buildList {
                    while (rs.next()) add(
                        SubCategory(
                            id = rs.getInt("Id"), categoryId = rs.getInt("CategoryId"),
                            categoryName = rs.getString("CategoryName") ?: "", name = rs.getString("Name") ?: "",
                            description = rs.getString("Description"), itemCount = rs.getInt("ItemCount"),
    ...
    private fun categoryOwned(conn: Connection, categoryId: Int): Boolean =
        conn.prepareStatement("SELECT 1 FROM Categories WHERE Id = ? AND UserId = ?").use { ps ->
            ps.setInt(1, categoryId); ps.setInt(2, Session.userId); ps.executeQuery().use { it.next() }
        }

    suspend fun create(categoryId: Int, name: String, description: String?): Result<Int> = Db.withConnection { conn ->
        if (!categoryOwned(conn, categoryId)) return@withConnection Result.failure(IllegalArgumentException("Category not found."))
        conn.prepareStatement(
            "INSERT INTO SubCategories (UserId, CategoryId, Name, Description, CreatedAt, IsActive) VALUES (?, ?, ?, ?, datetime('now'), 1)"
        ).use { ps ->
            ps.setInt(1, Session.userId); ps.setInt(2, categoryId); ps.setString(3, name.trim()); ps.setString(4, description?.trim())
            ps.executeUpdate()
        }
        Result.success(Sql.lastInsertId(conn))
    }

    suspend fun update(id: Int, categoryId: Int, name: String, description: String?): Result<Unit> = Db.withConnection { conn ->
        if (!categoryOwned(conn, categoryId)) return@withConnection Result.failure(IllegalArgumentException("Category not found."))
        conn.prepareStatement(
            "UPDATE SubCategories SET CategoryId = ?, Name = ?, Description = ? WHERE Id = ? AND UserId = ?"
        ).use { ps ->
            ps.setInt(1, categoryId); ps.setString(2, name.trim()); ps.setString(3, description?.trim())
            ps.setInt(4, id); ps.setInt(5, Session.userId); ps.executeUpdate()
        }
        Result.success(Unit)
    }
