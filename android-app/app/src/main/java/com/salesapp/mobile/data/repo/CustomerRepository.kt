...
/** CRUD + profile aggregation for customers (local SQLite), mirroring CustomersController + OrderMath. */
class CustomerRepository {
    ...
    suspend fun list(
        ...
        val sql = """
            ...
            $where ORDER BY c.CreatedAt DESC, c.Id DESC LIMIT ?, ?
        """.trimIndent()
    ...
    suspend fun create(name: String, phone: String?, email: String?, address: String?, routeId: Int?): Result<Int> = Db.withConnection { conn ->
        if (routeId != null && !routeOwned(conn, routeId)) return@withConnection Result.failure(IllegalArgumentException("Route not found."))
        conn.prepareStatement(
            "INSERT INTO Customers (UserId, Name, Phone, Email, Address, RouteId, CreatedAt, IsActive) VALUES (?, ?, ?, ?, ?, ?, datetime('now'), 1)"
        ).use { ps ->
    ...
    suspend fun update(id: Int, name: String, phone: String?, email: String?, address: String?, routeId: Int?): Result<Unit> = Db.withConnection { conn ->
        if (routeId != null && !routeOwned(conn, routeId)) return@withConnection Result.failure(IllegalArgumentException("Route not found."))
        conn.prepareStatement(
            "UPDATE Customers SET Name = ?, Phone = ?, Email = ?, Address = ?, RouteId = ? WHERE Id = ? AND UserId = ?"
        ).use { ps ->
            ps.setString(1, name.trim()); ps.setString(2, phone?.trim()); ps.setString(3, email?.trim()); ps.setString(4, address?.trim())
    ...
    suspend fun details(customerId: Int): CustomerSearchResult? = Db.withConnection { conn ->
        val customer = conn.prepareStatement(
            """
            SELECT c.Id, c.Name, c.Phone, c.Email, c.Address, c.RouteId, r.Name AS RouteName, c.CreatedAt, c.IsActive
            FROM Customers c LEFT JOIN Routes r ON r.Id = c.RouteId WHERE c.Id = ? AND c.UserId = ?
            """.trimIndent()
        ).use { ps ->
    ...
    suspend fun advance(customerId: Int): CustomerAdvance = Db.withConnection { conn ->
        CustomerAdvance(loadCustomerOrders(conn, customerId).sumOf { overpaidOn(it) })
    }
    ...
    private fun buildDetails(conn: Connection, c: Customer): CustomerSearchResult {
        ...
        val overall = when { totalPaid <= 0 -> "Pending"; totalRemaining <= 0 -> "Paid"; else -> "Advance" }
        ...
        return CustomerSearchResult(
            customer = c, totalOrdered = totalOrdered, totalPaid = totalPaid, totalRemaining = totalRemaining,
            orderPayments = orderPayments, advanceBalance = advanceBalance, advanceUsed = advanceUsed,
            overallPaymentStatus = overall, pendingOrders = pending, deliveredOrders = delivered, orders = active,
        )
    }
    ...
    private fun loadCustomerOrders(conn: Connection, customerId: Int): List<Order> =
        conn.prepareStatement(
            """
            SELECT Id, OrderNumber, OrderDate, DeliveryDate, Status, PaymentStatus, Source, TotalAmount, PaidAmount, RemainingAmount, Notes
            FROM Orders WHERE CustomerId = ? AND IsActive = 1 ORDER BY OrderDate DESC
            """.trimIndent()
        ).use { ps ->
            ...
            ps.executeQuery().use { rs ->
                buildList {
                    while (rs.next()) add(
                        Order(
                            id = rs.getInt("Id"), orderNumber = rs.getString("OrderNumber") ?: "",
                            customerId = customerId, customerName = "",
                            orderDate = rs.getString("OrderDate"), deliveryDate = rs.getString("DeliveryDate"),
                            ...
                            totalAmount = rs.getDouble("TotalAmount"), paidAmount = rs.getDouble("PaidAmount"),
                            remainingAmount = rs.getDouble("RemainingAmount"), notes = rs.getString("Notes"),
                        )
                    )
                }
            }
    ...
    private fun advanceUsed(conn: Connection, customerId: Int): Double =
        conn.prepareStatement(
            "SELECT COALESCE(SUM(p.Amount), 0) FROM Payments p INNER JOIN Orders o ON o.Id = p.OrderId " +
                "WHERE o.CustomerId = ? AND o.IsActive = 1 AND p.Method = 'Advance'"
        ).use { ps -> ps.setInt(1, customerId); ps.executeQuery().use { if (it.next()) it.getDouble(1) else 0.0 } }

    private fun overpaidOn(o: Order): Double =
        if (o.status == OrderStatus.Cancelled) max(0.0, o.paidAmount) else max(0.0, o.paidAmount - o.totalAmount)

    private fun mapCustomer(rs: java.sql.ResultSet): Customer {
        val routeId = rs.getInt("RouteId").let { if (rs.wasNull()) null else it }
        return Customer(
            id = rs.getInt("Id"), name = rs.getString("Name") ?: "", phone = rs.getString("Phone"),
            email = rs.getString("Email"), address = rs.getString("Address"), routeId = routeId,
            routeName = rs.getString("RouteName"), createdAt = rs.getString("CreatedAt"), isActive = rs.getBoolean("IsActive"),
        )
    }
